...
1. Count the number of quantified proteins for each group, gender and
...
2. Experiment with `goem_line` to produce line plots showing the
   expression data for a subset of samples (below I used those
   starting with `"P1F"`). You will need to **group** proteins for the
   same protein data points in different samples to be linked.

```{r ggex2, echo=FALSE}
ggplot(data = filter(tbl, grepl("P1F", tbl$Sample)),
	   aes(x = Sample, y = expression, colour = Protein)) +
...
3. Plot the expression of all proteins in one sample (for example
