...
upper right and lower left triangles of the matrix. Each can be
populated independently by setting the `upper.panel` and `lower.panel`
arguments (see `?pairs`). In addition, the `diag.panel` can be set to
set the panels along the diagonal. The upper and lower panels are set
by a function with signature `function(x, y, ...)` while the diagonal
panel is controled by a function with signature `function(x, ...)`.

> **Challenge**
>
> Update the output of `pairs` by defining the upper and lower panels
> differently. You can use the `panel.smooth` or define your own
> function. See also the `pairs` documentation page for a `panel.cor`
> function that displays the correlation.

<details>
```{r pairsln}
my.panel <- function(x, y, ...) {
    points(x, y, col = "#00000030", ...)
    grid()
    abline(0, 1, col = "red")
}
pairs(iprg3[, 2:5], upper.panel = my.panel, log = "xy")
```
</details>

See also `lattice::splom` and `GGally::ggairs` for alternatives.

A general technique to overcome overplotting is to set the alpha scale
(transparency), to use `graphics::smoothScatter`, or to use binning
(see `bexbin` and `geom_hex`):
...
It we want to add the line that models the data linearly, we can
calculate the parameters using the `lm` function:
...
<details>
```{r anscombechallenge, echo = FALSE}
x3 <- anscombe[, 3]
y3 <- anscombe[, 7]
lmod <- lm(y3 ~ x3)
summary(lmod)
...
Write a panel function that calculates and plots the linear model
between two samples. Bonus: highlight points that have extreme
residuals.
...
my.panel <- function(x, y, ...) {
	...
	y <- y[sel]
	lmod <- lm(y ~ x)
	r <- residuals(lmod)/predict(lmod)
	q1 <- quantile(r, 0.01)
	q2 <- quantile(r, 0.99)
	inq <- r > q1 & r < q2
	points(x, y, col = ifelse(inq, "black", "steelblue"))
	grid()
	abline(lmod, col = "red")
}
pairs(log10(iprg3[, 2:5]), upper.panel = my.panel)
```
<details>
...
```{r volc3, echo=FALSE}
i <- c(713L, 714L, 756L, 2456L, 7081L, 7082L, 7094L, 9823L, 10299L, 10604L)
lfc <- fData(ALL_bcrneg)$logFC
bh <- fData(ALL_bcrneg)$adj.P.Val
sign <- abs(lfc) > 1 & bh < 0.01
plot(lfc, -log10(bh),
...
### Treating the plot as a statistic

From

> Mahbubul Majumder, Heike Hofmann & [Dianne
> Cook](http://www.dicook.org/) (2013) Validation of Visual
> Statistical Inference, Applied to Linear Models, Journal of the
> American Statistical Association, 108:503, 942-956, DOI:
> [10.1080/01621459.2013.808157](https://www.tandfonline.com/doi/full/10.1080/01621459.2013.808157?scroll=top&needAccess=true).


![](./img/uasa_a_808157_o_f0004g.jpg)

**Figure 4** Lineup plot (m = 20) using scatterplots for testing H 0:
β k = 0, where covariate Xk is continuous. One of these plots is the
plot of the actual data, and the remaining are null plots, produced by
simulating data from a null model that assumes H 0 is true. Which plot
is the most different from the others, in the sense that there is the
steepest slope? (The position of the actual data plot is provided in
Section 5.2.) The online version of this figure is in color.


> **Challenge**
>
> Randomise the BCR/ABL and NEG labels in the `ALL_bcrneg` dataset and
> repeat the statistical analysis as described in the *Data*
> chapter. Repeat this 11 times and compare the p-value histograms of
> the actual data and the randomised replicates. Can you spot the
> original data?

<details>
```{r randompvals, fig.width = 10, fig.height=8}
f <- function(x) {
	require("limma")
	## randomlise groups
	fData(x) <- fData(x)[, numeric()]
	x$mol.biol <- sample(x$mol.biol)

	design <- model.matrix(~ 0 + x$mol.biol)
...
res <- replicate(11, f(ALL_bcrneg))
res <- cbind(res, fData(ALL_bcrneg)$P.Value)
res <- res[, sample(ncol(res))]

par(mfrow = c(3, 4))
tmp <- apply(res, 2, hist, main = "")
```
</details>
...
> Challenge
>
> Apply hierarchical clustering on the `mulvey2015norm` data and
> generate a sample dendrogram using the dedicated `plot` method as
> shown below.
...
cut it at a distance around 15, that would produce 3 clusters, namely
the XEN samples, the late time points 72 and 48 hours, and the early
time points.

```{r cuthcl, echo=FALSE, fig.cap = "Cutting the dendrogram at height 15."}
plot(hcl)
abline(h = 15, col = "red")
```
...
- cut the tree at a specific height: `cutree(hcl, h = 15)`
- cut the tree to get a certain number of clusters: `cutree(hcl, k = 3)`

> Challenge
>
> - Cut the hierarchical clustering result at a height to obtain
>   4 clusters by setting `h`.
>
> - Cut the hierarchical clustering result at a height to obtain
>   4 clusters by setting directly `k`, and verify that both provide
>   the same results.
>
> - What happens if we wanted to obtain 5 clusters.

<details>
```{r cuthclsol, eval=FALSE}
plot(hcl)
abline(h = 11, col = "red")
cutree(hcl, k = 4)
cutree(hcl, h = 11)
identical(cutree(hcl, k = 4), cutree(hcl, h = 11))
plot(hcl, labels = cutree(hcl, k = 4))
```
</details>

For more advanced dendrogram visualisations, see `r CRANpkg("dendextend")`.
...
```{r allsignhcl, fig.height = 4, fig.width = 10}
...
Produce a hierarchical cluster showing relations between mean
organelle profiles from the `hlm` data, such as shown below. To
colour the labels, convert the `hclust` object to a dendrogram with
`as.dendrogram` and set the label colours using `set(dendrogram,
"labels_colors", col_vector))` from the
[`dendextend`](https://github.com/talgalili/dendextend/) package.

```{r makehclust, echo=FALSE}
par(mar = c(15, 3, 0, 0))
pRoloc::mrkHClust(hlm)
```

<details>
```{r mkrhclust, eval=FALSE}
x <- exprs(hlm)
cm <- by(x, fData(hlm)$markers, colMeans)
x <- do.call(rbind, as.list(cm))
plot(hclust(dist(x)))
```
</details>
...
>   dataset, save the results in a new variable `cl`, and explore its
>   output when printed. Set the number of clusters to be the number
>   of annotated sub-cellular niches.
>
...
>   output. Use it to colour the inferred clusters to generate a
>   figure like that shown below.

<details>
```{r solkmplot, echo=TRUE}
...
Below, we see that if we repeat k-means on our `x` data multiple
times, setting the number of iterations to 1 or greater and check
whether you repeatedly obtain the same results.
...
cl3 <- kmeans(x, centers = 3, nstart = 1)
...
proteins from the `mulvey2015norm` data in 16 clusters.

1. Use k-means to cluster the `mulvey2015norm` data, setting `centers
   = 20`. Make sure you repeat the clustering more than once (setting
   `nstart`).
...
tb <- gather(x, key = sample, value = expression,
			 -cluster, -Accession) %>%
	as_tibble
...
able to retain about 75% of the total variance in the data.
...
```{r mulveybiplot, fig.cap=""}
biplot(pca)
```

Feature vectors that are in the same direction in PC space are also
correlated in the original data space. This can be seen below when
running PCA on the protein (rather than the samples).

```{r pca2}
biplot(prcomp(MSnbase::exprs(mulvey2015norm)))
```
...
> Challenge
>
> - Repeat the PCA analysis above, reproducing the biplot and
>   preparing a barplot of the percentage of variance explained by
>   each PC.
>
...
```{r pcax, echo=FALSE, fig.width=12, fig.cap=""}
par(mfrow = c(1, 2))
plot(pca$x[, 1:2], col = mulvey2015norm$times)
plot(pca$x[, 3:4], col = mulvey2015norm$times)
```

<details>
```{r pcaxcol, eval=FALSE}
par(mfrow = c(1, 2))
plot(pca$x[, 1:2], col = mulvey2015norm$times)
plot(pca$x[, 3:4], col = mulvey2015norm$times)
```
...
Produce the PCA plot for the `ALL_bcrneg` samples, and annotate the
...
Repeat the kmeans clustering analysis on the `hml` data above and
visualise the results on a PCA plot.

<details>
```{r pcachallenge2}
cl <- kmeans(MSnbase::exprs(hlm), length(getMarkerClasses(hlm)), nstart = 10)
pca <- prcomp(MSnbase::exprs(hlm), scale = TRUE, center = TRUE)
plot(pca$x[, 1:2], col = cl$cluster)
```
</details>


Produce a PCA with the proteins of the complete `hyperLopit2015`
experiment. Label the results using `final.assignment` feature
variable (corresponding to the proteins assignment to they
sub-cellular localisation) and scale to size of the points based on
the assignment scores available in the `svm.score` feature variable
(this might need to transform the scores to make differences
visisble).
...
The algorithm is stochastic, and will produce different results at
each repetition.
...
> **Challenge**
>
> Apply PCA and t-SNE to a data set (for example the `hyperLOPIT2015`
> or `hlm` data).
...
library("Rtsne")
pca <- prcomp(MSnbase::exprs(hlm), scale = TRUE, center = TRUE)
rtsne <- Rtsne(MSnbase::exprs(hlm))

par(mfrow = c(1, 2))
plot(pca$x[, 1:2], main = "PCA")
plot(rtsne$Y[, 1:2], main = "t-SNE")
```
</details>
...
> National Science Review, Volume 1, Issue 2, 1 June 2014, Pages
> 293–314,
> [https://doi.org/10.1093/nsr/nwt032](https://academic.oup.com/nsr/article/1/2/293/1397586).
...
- Informative data should show a good separation between the two
  classes.
...
- Simulate = 60 observations.
- Each observation is in d = 800, 6400 dimensions
- Left: max absolute correlation the first dimension and any other
  dimension
- Right: max absolute correlation the first dimension and a linear
  combination of any 4

![](./img/fan-nsr-2.png)

See [this](https://www.nature.com/articles/nri.2017.150) comment
scientific correspondance for a recent discussion related to flow and
mass cytometry data.
...
pca <- prcomp(t(MSnbase::exprs(ALL_bcrneg)), scale = TRUE, center = TRUE)
par(mfrow = c(1, 2))
plot(pca$x[,1:2], col = ALL_sign$mol.biol, pch = 19)
legend("bottomleft", legend = unique(ALL_sign$mol.biol),
	   col = 1:2, pch = 19, bty = "n")
smoothScatter(pca$rotation[, 1], fData(ALL_bcrneg)$adj.P.Val,
...
Finally, the `r Githubpkg("talgalili/heatmaply")` and
`r Githubpkg("rstudio/d3heatmap")` packages can be used to
